...
int byteSwap(int x, int n, int m) {
    int clear_mask, byte_n, byte_m;
    n <<= 3, m <<= 3; // Convert byte indices to bit indices
    clear_mask = ~((0xFF << n) | (0xFF << m)); // Clear the nth and mth bytes
    byte_n = (x >> n) & 0xFF; // Extract the nth byte
    byte_m = (x >> m) & 0xFF; // Extract the mth byte
    return (x & clear_mask) | (byte_n << m) | (byte_m << n); // Swap and combine
}
/* 
...
unsigned float_i2f(int x) {
    ...
    exp = x_exp + 0x7f;
    x <<= (31 - x_exp); // Shift x to the left so that the highest set bit is at the 31st position
    frac = (x >> 8) & frac_mask; // Get the 23 bits after the highest set bit
    round = x & 0xff;
    frac += round > 0x80 || (round == 0x80 && (frac & 1));
    if (frac >> 23) { // If frac overflows, we need to adjust
        frac &= frac_mask;
        exp++;
    }
...
int float64_f2i(unsigned uf1, unsigned uf2) {
    ...
    // In general, the left most bit of 1 's position index is equal to the exponent.
    // So we need to shift the fraction to the right by (31 - exp) to get the integer value.
    frac = frac >> (31 - exp); // Shift the fraction to the right that the highest set bit is at the exp-th position
